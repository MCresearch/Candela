...
nonHB_correlation3::nonHB_correlation3(){}

nonHB_correlation3::~nonHB_correlation3(){}

void nonHB_correlation3::Routine()
{
    ...
    for (int iHB=0; iHB<INPUT.nPT; iHB++)
    {
        ...
        nonHB_start_time[iHB] = new double[ndt1];
...
void nonHB_correlation3::calc(CellFile &cel)
{
    int ito=-1;
	int ith=-1;
	int itc=-1;
    ...
    for (int iwater1=0; iwater1<cel.atom[ito].na-1; iwater1++)
    {
        for (int iwater2=iwater1+1; iwater2<cel.atom[ito].na; iwater2++)
        {
            ...
            if (dist_oo < INPUT.rcut_oo)
            {
                ...
                if (index_of_HB >= 0)
                {
                    if (nonHB_correlation::HB_bonded(water, iwater1, iwater2))
                    {
                    ...
                    {
                        if (this->nonHB_nrecord[index_of_HB] < this->ndt1)
                        {
                            this->nonHB_start_time[index_of_HB][this->nonHB_nrecord[index_of_HB]] = cel.snapshot_time;
                            this->nonHB_nrecord[index_of_HB]++;
                        }
                    }
                }
                else if (this->nHB < INPUT.nPT)
                {
                    ...
                    //cout << this->HB_water_index[this->nHB][0] << " " << this->HB_water_index[this->nHB][1] << endl;
                    //cout << nonHB_correlation::HB_bonded(water, iwater1, iwater2) << endl;
                    if (nonHB_correlation::HB_bonded(water, iwater1, iwater2))
                    {
                        this->HB_start_time[this->nHB][0] = cel.snapshot_time;
                        this->HB_nrecord[this->nHB] = 1;
                        //cout << this->HB_nrecord[this->nHB] << endl;
                    }
                    else
                    {
                        this->nonHB_start_time[this->nHB][0] = cel.snapshot_time;
                        this->nonHB_nrecord[this->nHB] = 1;
                    }
                    this->nHB++;
                }
            }
...
void nonHB_correlation3::output()
{
    ...
    for (int iHB=0; iHB<this->nHB; iHB++)
    {
        if (this->nonHB_nrecord[iHB] > 0 and this->HB_nrecord[iHB] > 0)
        {
            for (int idt=0; idt<this->ndt; idt++) tcf[idt] = 0;
            for (int irecord1=0; irecord1<this->HB_nrecord[iHB]; irecord1++)
            {
                for (int irecord2=0; irecord2<this->nonHB_nrecord[iHB]; irecord2++)
                {
                    if (this->HB_start_time[iHB][irecord1] >= this->nonHB_start_time[iHB][irecord2])
                    {
                        int which_time = int(round((this->HB_start_time[iHB][irecord1] - this->nonHB_start_time[iHB][irecord2])/INPUT.msd_dt));
                        assert(which_time >= 0);
            ...
            {
                tcf[idt] /= this->nonHB_nrecord[iHB];
